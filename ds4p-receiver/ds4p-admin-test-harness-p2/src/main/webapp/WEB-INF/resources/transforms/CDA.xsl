...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
		...
		<html>
			<head>
				...
				<xsl:comment> Do NOT edit this HTML directly, it was generated via an XSLt
					transformation from the original release 2 CDA Document. </xsl:comment>
				<title>
					<xsl:value-of select="$title"/>
				</title>
			</head>
			<xsl:comment> Derived from HL7 Finland R2 Tyylitiedosto: Tyyli_R2_B3_01.xslt Updated by
				Calvin E. Beebe, Mayo Clinic - Rochester Mn. </xsl:comment>
			<body>
				...
				<h2 align="center" style="color:red">
					<xsl:value-of select="concat('CONFIDENTIALITY:', n1:confidentialityCode/@code)"
					/>
				</h2>
				<p align="center">
					<b>
	...
	<xsl:template name="getAddress">
		...
		<br/><xsl:value-of select="$addr/n1:city"/>, <xsl:value-of select="$addr/n1:state"/>,
...
-->
	...
	<xsl:template match="n1:title">
		...
		<h3>
			...
			<xsl:text>  </xsl:text>
			<span style="font-weight:bold;color:red">[CONFIDENTIALITY: <xsl:value-of
					select="preceding-sibling::n1:confidentialityCode/@code"/>]</span>
		</h3>
	...
	<xsl:template match="n1:tr">
		<tr>
			...
			<xsl:if test="not(xenc:EncryptedData)">
				<xsl:copy-of select="@*"/>
				<xsl:apply-templates/>
			</xsl:if>
			<xsl:if test="xenc:EncryptedData">
				<td><span style="font-weight:bold;color:red">[MASKED ENTRY]</span></td>
				<td></td>
				<td></td>
				<td></td>
			</xsl:if>
	...
	<xsl:template match="n1:td">
		<td>
			<xsl:if test="xenc:EncryptedData">
				<span style="font-weight:bold;color:red">[MASKED ENTRY]</span>
			</xsl:if>
			<xsl:if test="not(xenc:EncryptedData)">
				<xsl:copy-of select="@*"/>
				<xsl:apply-templates/>
			...
			<xsl:choose>
				<xsl:when test="contains(., 'Acute')">
					<span style="font-weight:bold;color:red"><xsl:text>  </xsl:text>[ENTRY
							METADATA:<xsl:value-of
							select="ancestor::n1:section//n1:externalDocument/n1:text/n1:reference/@value"
						/>]</span>

				</xsl:when>

				<xsl:when test="contains(., 'Substance')">
					<span style="font-weight:bold;color:red"><xsl:text>  </xsl:text>[ENTRY
							METADATA:<xsl:value-of
							select="ancestor::n1:section//n1:externalDocument/n1:text/n1:reference/@value"
						/>]</span>
				...
				<xsl:when test="contains(., 'Bupropion')">
					<span style="font-weight:bold;color:red"><xsl:text>  </xsl:text>[ENTRY
							METADATA:<xsl:value-of
							select="ancestor::n1:section//n1:externalDocument/n1:text/n1:reference/@value"
						/>]</span>
